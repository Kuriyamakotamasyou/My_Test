...
void LCD_STRing(uint16_t width, uint16_t hight, uint8_t* p, uint16_t BackColor, uint16_t BACKCOLOR, uint16_t POINTCOLOR){
	ST7735_LCD_Driver.FillRect(&st7735_pObj, 0, 0, ST7735Ctx.Width,ST7735Ctx.Height, BackColor);
	uint8_t text[20];
	sprintf((char *)&text, p);
	LCD_ShowString(width, hight, ST7735Ctx.Width, 16, 16, text, BACKCOLOR, POINTCOLOR);
}
...
void LCD_ShowChar(uint16_t x,uint16_t y,uint8_t num,uint8_t size,uint8_t mode, uint16_t BACKCOLOR, uint16_t PONITCOLOR)
{  							  
  uint8_t temp,t1,t;
	uint16_t y0=y;
	uint16_t x0=x;
	uint16_t colortemp=PONITCOLOR; 
  uint32_t h,w;
  ...
  ST7735_GetXSize(&st7735_pObj,&w);
	...
	if(!mode) //非叠加方式
	{
		...
		{   
			...
			{			    
				if(temp&0x80)
					PONITCOLOR=(colortemp&0xFF)<<8|colortemp>>8;
				else 
					PONITCOLOR=(BACKCOLOR&0xFF)<<8|BACKCOLOR>>8;
				
				write[count][t/2]=PONITCOLOR;
				count ++;
				if(count >= size) count =0;
				
				temp<<=1;
				y++;
				if(y>=h){PONITCOLOR=colortemp;return;}//超区域了
				if((y-y0)==size)
				{
					y=y0;
					x++;
					if(x>=w){PONITCOLOR=colortemp;return;}//超区域了
					break;
				}
			}
		}
	...
	else//叠加方式
	{
		...
		{   
			...
			{			    
				if(temp&0x80)
					write[count][t/2]=(PONITCOLOR&0xFF)<<8|PONITCOLOR>>8;
				count ++;
				if(count >= size) count =0;
...
//size:字体大小
//*p:字符串起始地址
void LCD_ShowString(uint16_t x,uint16_t y,uint16_t width,uint16_t height,uint8_t size,uint8_t *p, uint16_t BACKCOLOR, uint16_t PONITCOLOR)
{         
	uint8_t x0=x;
	width+=x;
	height+=y;
    ...
    {       
        ...
        LCD_ShowChar(x,y,*p,size,0,BACKCOLOR,PONITCOLOR);
        x+=size/2;
        p++;
    }  
}
